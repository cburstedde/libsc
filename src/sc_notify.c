...
#define SC_NOTIFY_FUNC_SNAP(notify,snap)                   \
do {                                                       \
  if (notify->stats) {                                     \
    SC_FUNC_SNAP (notify->stats, &(notify->flop), (snap)); \
  }                                                        \
} while (0)

#define SC_NOTIFY_FUNC_SHOT(notify,snap)                   \
do {                                                       \
  if (notify->stats) {                                     \
    SC_FUNC_SHOT (notify->stats, &(notify->flop), (snap)); \
  }                                                        \
} while (0)
...
sc_notify_new (sc_MPI_Comm comm)
{
  ...
  sc_flops_start_nopapi (&(notify->flop));
  return notify;
}
...
sc_statistics_t    *
sc_notify_get_stats (sc_notify_t * notify)
{
  return notify->stats;
}
                                             ...
                                             sc_MPI_Comm))
{
  ...
  SC_NOTIFY_FUNC_SNAP (notify, &snap);
  ...
  if (in_payload) {
    ...
    if (out_payload) {
      ...
      rpayload = (char *) out_payload->array;
    }
    else {
    ...
    if (!out_payload) {
      ...
      sc_array_resize (in_payload, num_senders);
      memcpy ((char *) in_payload->array, rpayload, num_senders * msg_size);
      SC_FREE (rpayload);
  ...
  if (sorted && !sc_array_is_sorted (senders, sc_int_compare)) {
    if (!out_payload) {
      ...
      sc_array_t         *sorter;
      size_t              msg_size = out_payload->elem_size;
      int                 j;
...
/** Complete sc_notify_payload() using a function that gives a census
 * of the number of senders */
static void
sc_notify_payload_census (sc_array_t * receivers, sc_array_t * senders,
                          sc_array_t * in_payload, sc_array_t * out_payload,
                          int sorted, sc_notify_t * notify,
                          int (*census_fn) (sc_array_t *,
                                            sc_notify_t * notify))
{
  int                 i;
  ...
  num_senders = (*census_fn) (receivers, notify);
  ...
  for (i = 0; i < num_receivers; i++) {
    ...
    mpiret = sc_MPI_Isend (&cpayload[i * msg_size], msg_size, sc_MPI_BYTE,
                           ireceivers[i], SC_TAG_NOTIFY_CENSUS, mpicomm,
                           &sendreqs[i]);
    SC_CHECK_MPI (mpiret);
  }
  for (i = 0; i < num_senders; i++) {
    ...
    mpiret =
      sc_MPI_Recv (&crecv[i * stride + sizeof (int)], msg_size, sc_MPI_BYTE,
                   sc_MPI_ANY_SOURCE, SC_TAG_NOTIFY_CENSUS, mpicomm, &status);
    SC_CHECK_MPI (mpiret);
  ...
  if (!msg_size && senders) {
    if (in_payload && out_payload == NULL) {
      sc_array_reset (in_payload);
      out_payload = in_payload;
    }
    if (out_payload) {
      sc_array_resize (out_payload, num_senders);
    }
    SC_NOTIFY_FUNC_SHOT (notify, &snap);
    return;
  }
...
/** Finish array logic after data is received */
static void
sc_notify_payload_cleanup (sc_array_t * senders, sc_array_t * recv_buf,
                           sc_array_t * in_payload, sc_array_t * out_payload,
                           int sorted)
{
  int                 num_senders;
  int                *isenders;
  int                 i;
  int                 msg_size = in_payload ? (int) in_payload->elem_size : 0;
  char               *cpayload;
                            ...
                            int sorted, sc_notify_t * notify)
{
  ...
  sc_notify_payload (receivers, first_senders, sizes, NULL, sorted, notify);
...
static void
sc_notify_payloadv_census (sc_array_t * receivers, sc_array_t * senders,
                           sc_array_t * in_payload, sc_array_t * out_payload,
                           sc_array_t * in_offsets, sc_array_t * out_offsets,
                           int sorted, sc_notify_t * notify,
                           void (*censusv_fn) (sc_array_t *, sc_array_t *,
                                               int *, sc_notify_t *))
{
  int                 i;
  int                 num_senders, num_receivers;
  ...
  (*censusv_fn) (receivers, in_offsets, &num_senders_size[0], notify);
  ...
  for (i = 0; i < num_receivers; i++) {
    mpiret =
      sc_MPI_Isend (&cpayload[inoff[i] * msg_size],
                    ...
                    ireceivers[i], SC_TAG_NOTIFY_CENSUSV, mpicomm,
                    &sendreqs[i]);
    SC_CHECK_MPI (mpiret);
  }
  ...
  for (i = 0; i < num_senders; i++) {
    ...
    mpiret =
      sc_MPI_Recv (&crecv[outoff[i] * msg_size],
                   ...
                   sc_MPI_ANY_SOURCE, SC_TAG_NOTIFY_CENSUSV, mpicomm,
                   &status);
    SC_CHECK_MPI (mpiret);
  ...
  if (length > 1) {
    ...
    *array = *(sc_array_t *) sc_array_index_int (&recvbufs, 0);
    ...
    mpiret = sc_MPI_Waitall
      (divn, (sc_MPI_Request *) sendreqs.array, sc_MPI_STATUSES_IGNORE);
    SC_CHECK_MPI (mpiret);
                        ...
                        sc_notify_t * notify)
{
  ...
  if (in_payload) {
    size_t              lowbound =
      (in_payload->elem_size >
       sizeof (int)) ? (in_payload->elem_size - sizeof (int)) : 0;
...
sc_notify_payload_pex (sc_array_t * receivers, sc_array_t * senders,
                       ...
                       sc_array_t * out_payload, sc_notify_t * notify)
{
  int                 i;
  ...
  for (i = 0; i < num_receivers; i++) {
    ...
    if (in_payload) {
      memcpy (&buffered_receivers[stride * ireceivers[i] + 1],
              sc_array_index_int (in_payload, i), in_payload->elem_size);
    }
  }
  ...
  if (out_payload) {
    sc_array_resize (out_payload, (size_t) found_num_senders);
  }
  ...
  for (i = 0; i < mpisize; i++) {
    if (all_receivers[stride * i + 0]) {
      if (out_payload) {
        memcpy (sc_array_index_int (out_payload, found_num_senders),
                &all_receivers[stride * i + 1], out_payload->elem_size);
      }
...
sc_notify_census_pcx (sc_array_t * receivers, sc_notify_t * notify)
{
  ...
  return num_senders;
#else
  SC_ABORT ("pcx implementation of sc_notify requires MPI-2.2 or greater");
#endif
}
...
sc_notify_census_rsx (sc_array_t * receivers, sc_notify_t * notify)
{
  ...
  mpiret = MPI_Alloc_mem (sizeof (int), MPI_INFO_NULL, &inum_senders);
  SC_CHECK_MPI (mpiret);
  inum_senders[0] = 0;
  mpiret =
    MPI_Win_create (inum_senders, sizeof (int), sizeof (int), MPI_INFO_NULL,
                    mpicomm, &win);
  SC_CHECK_MPI (mpiret);

  mpiret = MPI_Win_fence (MPI_MODE_NOPRECEDE, win);
  SC_CHECK_MPI (mpiret);
  ...
  for (i = 0; i < num_receivers; i++) {
    ...
    SC_ASSERT (j >= 0 && j < mpisize);
    mpiret =
      MPI_Accumulate (&one, 1, MPI_INT, j, 0, 1, MPI_INT, MPI_SUM, win);
    SC_CHECK_MPI (mpiret);
  }

  mpiret = MPI_Win_fence (MPI_MODE_NOSTORE | MPI_MODE_NOSUCCEED, win);
  SC_CHECK_MPI (mpiret);

  mpiret = MPI_Win_free (&win);
  SC_CHECK_MPI (mpiret);

  num_senders = inum_senders[0];
  mpiret = MPI_Free_mem (inum_senders);

  SC_NOTIFY_FUNC_SHOT (notify, &snap);
  return num_senders;
#else
  SC_ABORT ("rsx implementation of sc_notify requires MPI-2 or greater");
#endif
}
                       ...
                       int *num_senders_size, sc_notify_t * notify)
{
  ...
  ireceivers = (int *) receivers->array;
  num_receivers = (int) receivers->elem_count;
  ioff = (int *) in_offsets->array;

  mpiret =
    MPI_Alloc_mem (2 * sizeof (int), MPI_INFO_NULL, &inum_senders_size);
  SC_CHECK_MPI (mpiret);
  inum_senders_size[0] = 0;
  inum_senders_size[1] = 0;
  mpiret =
    MPI_Win_create (inum_senders_size, 2 * sizeof (int), sizeof (int),
                    MPI_INFO_NULL, mpicomm, &win);
  SC_CHECK_MPI (mpiret);
  ...
  for (i = 0; i < num_receivers; i++) {
    ...
    summand[0] = 1;
    summand[1] = ioff[i + 1] - ioff[i];

    SC_ASSERT (j >= 0 && j < mpisize);
    mpiret =
      MPI_Accumulate (&summand[0], 2, MPI_INT, j, 0, 2, MPI_INT, MPI_SUM,
                      win);
    SC_CHECK_MPI (mpiret);
  }
  ...
  num_senders_size[0] = inum_senders_size[0];
  num_senders_size[1] = inum_senders_size[1];
  mpiret = MPI_Free_mem (inum_senders_size);
  SC_NOTIFY_FUNC_SHOT (notify, &snap);
#else
  SC_ABORT ("rsx implementation of sc_notify requires MPI-2 or greater");
#endif
                          ...
                          int sorted, sc_notify_t * notify)
{
  ...
  for (i = 0; i < num_procs; i++) {
    ...
    if (peer == rank) {
      my_pos = i;
      if (in_payload) {
        my_payload = SC_ALLOC (char, in_payload->elem_size);
        memcpy (my_payload, sc_array_index_int (in_payload, i),
                in_payload->elem_size);
      }
      continue;
    }
  ...
  for (i = 0; i < num_receivers_ranges; i++) {
    ...
    if (in_payload && pos > 0) {
      memcpy (&send[1], sc_array_index_int (in_payload, pos - 1),
              in_payload->elem_size);
    }
  ...
  for (i = 0; i < num_senders_ranges; i++) {
    ...
    if (recv[0]) {
      ...
      if (my_pos >= 0 && last_proc < rank && proc > rank) {
        sender[0] = rank;
        if (out_payload) {
          memcpy (sc_array_index_int (out_payload, num_senders),
...
static void
sc_notify_payload_superset (sc_array_t * receivers, sc_array_t * senders,
                            sc_array_t * in_payload, sc_array_t * out_payload,
                            int sorted, sc_notify_t * notify)
{
  int                 num_receivers, num_senders;
  ...
  char               *cpayload = NULL;
  int                 msg_size = 0;
  sc_MPI_Request     *sendreqs, *extrasendreqs;
  sc_MPI_Comm         comm;
  int                 queue;
  sc_array_t         *recv_buf = NULL;
  ...
  sc_notify_superset_get_callback (notify, &compute_superset, (void *) &ctx);
  SC_ASSERT (compute_superset);
  extra_receivers = sc_array_new (sizeof (int));
  super_senders = sc_array_new (sizeof (int));
  (*compute_superset) (receivers, extra_receivers, super_senders, notify,
                       ctx);
  num_extra_receivers = (int) extra_receivers->elem_count;

  extrasendreqs = SC_ALLOC (sc_MPI_Request, num_extra_receivers);
...
#endif
  num_super_senders = (int) super_senders->elem_count;

  if (!senders) {
    sc_array_reset (receivers);
    senders = receivers;
  }
  sc_array_resize (senders, (size_t) num_super_senders);
  sc_array_truncate (senders);

  if (msg_size) {
    if (sorted) {
      recv_buf =
        sc_array_new_count (msg_size + sizeof (int),
                            (size_t) num_super_senders);
      sc_array_truncate (recv_buf);
    }
    else {
      if (out_payload) {
        recv_buf = out_payload;
        ...
        recv_buf = sc_array_new (msg_size);
      }
      sc_array_resize (recv_buf, (size_t) num_super_senders);
      sc_array_truncate (recv_buf);
    }
  }

  for (queue = num_super_senders; queue > 0;) {
    int                 flag;
    sc_MPI_Status       status;

    mpiret =
      sc_MPI_Iprobe (sc_MPI_ANY_SOURCE, SC_TAG_NOTIFY_SUPER_TRUE, comm, &flag,
                     &status);
    SC_CHECK_MPI (mpiret);
    if (flag) {
      int                *r;
      ...
      j = status.MPI_SOURCE;
      SC_ASSERT (sc_array_bsearch (super_senders, &j, sc_int_compare) >= 0);
      if (sorted && msg_size) {
        r = (int *) sc_array_push (recv_buf);
        c = (char *) &r[1];
      }
        ...
        r = (int *) sc_array_push (senders);
        if (msg_size) {
          c = (char *) sc_array_push (recv_buf);
        }
      }
      r[0] = j;
      mpiret =
        sc_MPI_Recv (c, msg_size, sc_MPI_BYTE, j, SC_TAG_NOTIFY_SUPER_TRUE,
    ...
    mpiret =
      sc_MPI_Iprobe (sc_MPI_ANY_SOURCE, SC_TAG_NOTIFY_SUPER_EXTRA, comm,
                     &flag, &status);
    SC_CHECK_MPI (mpiret);
    if (flag) {
      j = status.MPI_SOURCE;
      ...
      mpiret =
        sc_MPI_Recv (NULL, 0, sc_MPI_BYTE, j, SC_TAG_NOTIFY_SUPER_EXTRA, comm,
  ...
  mpiret =
    sc_MPI_Waitall (num_extra_receivers, extrasendreqs,
                    sc_MPI_STATUSES_IGNORE);
  SC_CHECK_MPI (mpiret);
  SC_FREE (extrasendreqs);
  mpiret = sc_MPI_Waitall (num_receivers, sendreqs, sc_MPI_STATUSES_IGNORE);
  SC_CHECK_MPI (mpiret);
  SC_FREE (sendreqs);
                   ...
                   int sorted, sc_notify_t * notify)
{
  ...
  if (in_payload != NULL) {
    SC_ASSERT ((int) in_payload->elem_count == num_receivers);
    if (out_payload) {
      SC_ASSERT (SC_ARRAY_IS_OWNER (out_payload));
      SC_ASSERT (out_payload->elem_size == in_payload->elem_size);
    }
    else {
    ...
    SC_ASSERT (out_payload == NULL);
  }

  if (in_payload && in_payload->elem_size <= notify->eager_threshold) {
    first_in_payload = in_payload;
    first_out_payload = out_payload;
  }
  else {
    if (!senders) {
      receivers_copy =
  ...
  case SC_NOTIFY_PEX:
    ...
    break;
  case SC_NOTIFY_PCX:
    sc_notify_payload_census (receivers, senders, first_in_payload,
                              first_out_payload, sorted, notify,
                              sc_notify_census_pcx);
    break;
  case SC_NOTIFY_RSX:
    sc_notify_payload_census (receivers, senders, first_in_payload,
                              first_out_payload, sorted, notify,
  ...
  case SC_NOTIFY_RANGES:
    ...
    break;
  case SC_NOTIFY_SUPERSET:
    sc_notify_payload_superset (receivers, senders, first_in_payload,
                                first_out_payload, sorted, notify);
    break;
  default:
    SC_ABORT_NOT_REACHED ();
  ...
  if (in_payload && first_in_payload != in_payload) {
    ...
    if (out_payload) {
      sc_array_resize (out_payload, (size_t) num_senders);
      recv_payload = (char *) out_payload->array;
    }
    else {
    ...
    mpiret = sc_MPI_Waitall (num_receivers, sendreq, sc_MPI_STATUSES_IGNORE);
    SC_CHECK_MPI (mpiret);
    if (!out_payload) {
      sc_array_resize (in_payload, num_senders);
      memcpy (in_payload->array, recv_payload,
              (size_t) msg_size * num_senders);
                    ...
                    int sorted, sc_notify_t * notify)
{
  ...
  if (in_payload == NULL) {
    SC_ASSERT (out_payload == NULL && in_offsets == NULL
               && out_offsets == NULL);
    sc_notify_payload (receivers, senders, NULL, NULL, sorted, notify);
    SC_NOTIFY_FUNC_SHOT (notify, &snap);
    return;
  }
