                    ...
                    const double *errors, sc_amr_control_t * amr)
{
  ...
  amr->num_total_elements = si->count;
  amr->coarsen_threshold = 0.;
  amr->refine_threshold = 0.;
  amr->num_total_coarsen = 0;
  amr->num_total_refine = 0;
  amr->num_total_estimated = 0;
}

void
sc_amr_coarsen_threshold (sc_amr_control_t * amr,
                          ...
                          int max_binary_steps, void *user_data)
{
  ...
  double              coarsen_threshold_low, coarsen_threshold_high;
  ...
  num_total_low = num_local_ideal * amr->num_procs_long;
  coarsen_threshold_low = errors->min;
  coarsen_threshold_high = errors->max;
  if (coarsen_threshold_low >= coarsen_threshold_high ||
      num_total_elements + num_total_refine <= num_total_low) {

    SC_GLOBAL_STATISTICSF ("Binary search anomaly with low = %g, up = %g\n",
  ...
  num_total_high = (long) (num_total_low / binary_fudge);
  ...
  for (binary_count = 0; binary_count < max_binary_steps; ++binary_count) {
    ...
    SC_GLOBAL_LDEBUGF ("Total %ld estimated %ld coarsen %ld\n",
                       num_total_elements, num_total_estimated,
                       global_coarsen);
    SC_GLOBAL_INFOF ("Binary search for %ld elements at low = %g, up = %g\n",
                     num_total_estimated,
                     coarsen_threshold_low, coarsen_threshold_high);
  ...
  SC_GLOBAL_INFOF ("Binary search for coarsen stopped after %d steps"
                   " with threshold %g\n",
